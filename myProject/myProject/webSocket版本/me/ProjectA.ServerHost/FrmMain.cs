...
namespace Aviad.ProjectA.ServerHost
{
    ...
    public partial class FrmMain : Form
    {
        ...
        public Dictionary<Fleck.IWebSocketConnection, clientHeartbeat> clientTimers = new Dictionary<IWebSocketConnection, clientHeartbeat>();
        public Dictionary<Socket, serverHeartbeat> serverTimers = new Dictionary<Socket, serverHeartbeat>();
...
///---------------------------------------------------------------------------------
        ...
        public void closeClient(IWebSocketConnection clientSock)
        {
            Socket serverSock = null;
            if (myGlobals.ClientServer.ContainsKey(clientSock))
                serverSock = myGlobals.ClientServer[clientSock];
            if (wsServer.allSockets.Contains(clientSock))
        ...
        {
            Socket value;
            if (msgType == GlobalMessage.LOGIN)
            {
...
///---------------------------------------------------------------------------------
        ...
        private void createTerminal(int alPort)
        {
           // toClient = new ServerTerminal();
            ...
            //.MessageRecived += new TCPTerminal_MessageRecivedDel(ClientMessageRecived);
           ...
           wsServer.MessageRecived += new Server.onCliengMessageReceived(ClientMessageRecived);
            wsServer.ClientConnect += new Server.onClientConnected(ClientConnected);
            wsServer.ClientDisconnect += new Server.onClientDisconneted(ClientDisConnected);
            
            wsServer.startListening(alPort);
        }
         ...
         //   toSever.SendMessage(buffer);
          //  long value = socket.Handle.ToInt64();
           ...
           // myGlobals.psxClients[key].StartListen();
            ...
            serverHeartbeat sTimer = new serverHeartbeat();
            sTimer.timeout += new serverHeartbeat.timerDelegate(serverTimeout);
            sTimer.start();
