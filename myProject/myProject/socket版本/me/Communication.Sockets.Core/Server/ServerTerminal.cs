...
namespace Communication.Sockets.Core.Server
{
    public class ServerTerminal
    {
        ...
        public Dictionary<Socket, ConnectedClient> m_clients = 
            new Dictionary<Socket, ConnectedClient>();
            ...
            try
            {
                ...
                if (m_clients.ContainsKey(clientSocket))
                {
                    Debug.Fail(string.Format(
                        "Client with handle key '{0}' already exist!", clientSocket));
                }

               // m_clients[key] = connectedClient;
                  m_clients[clientSocket] = connectedClient;
                // create the call back for any client connections...
                m_socket.BeginAccept(new AsyncCallback(OnClientConnection), null);
        ...
        private void OnClientDisconnection(Socket socket)
        {
            ...
            {
                Debug.Fail(string.Format(
                    "Unknown client '{0}' has been disconncted!", socket));
            }
        }

        public void DistributeMessage(byte[] buffer, Socket key)
        {
            try
            {
